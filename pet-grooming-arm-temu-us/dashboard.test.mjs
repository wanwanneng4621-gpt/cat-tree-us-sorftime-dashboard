...
assert.match(html, /TEMU美国站市场分析/);
assert.match(html, /Amazon美国站市场分析/);
assert.match(html, /跨平台差异与证据迁移边界/);
assert.match(html, /TEMU美国站用户画像与需求假设/);
assert.match(html, /Amazon美国站用户需求与负评证据/);
assert.match(html, /Reddit外部场景与用户语言/);
assert.match(html, /七要素用户转化链/);
assert.match(html, /Kano需求与分平台落地/);
assert.match(html, /需求优先级 → 产品动作 → 页面证明/);
assert.match(html, /Amazon负评.*不能直接当作TEMU客户占比/s);
